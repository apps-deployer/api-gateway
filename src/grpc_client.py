...
class ProjectsGrpcClient:
    ...
    async def create_project(self, name: str, repo_url: str, deploy_config_template_id: str = "") -> Project:
        ...
        resp = await self._projects.CreateProject(req, metadata=self._metadata())
        return Project(id=resp.id, name=resp.name, slug=resp.slug, repo_url=resp.repo_url, owner_id=resp.owner_id)
    ...
    async def get_env(self, env_id: str) -> Env:
        resp = await self._envs.GetEnv(envs_pb2.GetEnvRequest(id=env_id), metadata=self._metadata())
        return Env(id=resp.id, name=resp.name, project_id=resp.project_id, target_branch=resp.target_branch)
    ...
    async def list_envs(self, project_id: str, limit: int = 100, offset: int = 0) -> list[Env]:
        ...
        return [Env(id=e.id, name=e.name, project_id=e.project_id, target_branch=e.target_branch) for e in resp.envs]
    ...
    async def delete_env(self, env_id: str) -> None:
        await self._envs.DeleteEnv(envs_pb2.DeleteEnvRequest(id=env_id), metadata=self._metadata())

    async def list_project_vars(self, project_id: str) -> list[Var]:
        resp = await self._vars.ListProjectVars(
            vars_pb2.ListProjectVarsRequest(project_id=project_id),
            metadata=self._metadata(),
        )
    ...
    async def update_project_var(self, var_id: str, value: str) -> None:
        await self._vars.UpdateProjectVar(
            vars_pb2.UpdateProjectVarRequest(id=var_id, value=value),
            metadata=self._metadata(),
        )

    async def delete_project_var(self, var_id: str) -> None:
        await self._vars.DeleteProjectVar(vars_pb2.DeleteProjectVarRequest(id=var_id), metadata=self._metadata())

    async def list_env_vars(self, env_id: str) -> list[Var]:
        resp = await self._vars.ListEnvVars(
            vars_pb2.ListEnvVarsRequest(env_id=env_id),
            metadata=self._metadata(),
        )
        return [Var(id=v.id, key=v.key) for v in resp.vars]
    ...
    async def update_env_var(self, var_id: str, value: str) -> None:
        await self._vars.UpdateEnvVar(
            vars_pb2.UpdateEnvVarRequest(id=var_id, value=value),
            metadata=self._metadata(),
        )

    async def delete_env_var(self, var_id: str) -> None:
        await self._vars.DeleteEnvVar(vars_pb2.DeleteEnvVarRequest(id=var_id), metadata=self._metadata())
    ...
    async def list_frameworks(self) -> list[Framework]:
        resp = await self._frameworks.ListFrameworks(
            frameworks_pb2.ListFrameworksRequest(limit=100, offset=0),
            metadata=self._metadata(),
        )
        return [
            Framework(
